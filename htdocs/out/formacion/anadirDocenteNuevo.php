<?php
////// SE LLAMA DESDE el JS checkRegistro.js; mediante llamada Ajax. COmprueba si un nombre de usuario existe en la BD, devuelve true si existe o false si no (o sea, el nombre está disponible y se puede tomar)
header("Content-type:application/json");
include("../../inc/inc.Settings.php");
include("../../inc/inc.Utils.php");
include("../../inc/inc.Init.php");
include("../../inc/inc.DBInit.php");
/////////////////////////////////////////////////////////////////////////////////////////////////////////// MAIN ////////////////////////////////////////////////////////////////////////////////////////////////

$nombre = $_GET['nombre']; //obtengo el id del ítem
$tel = $_GET['tel']; //obtengo el id del ítem
$correo = $_GET['correo']; //obtengo el id del ítem

											  
$settings = new Settings(); //acceder a parámetros de settings.xml con _antes
$driver=$settings->_dbDriver;
$host=$settings->_dbHostname;
$user=$settings->_dbUser;
$pass=$settings->_dbPass;
$base=$settings->_dbDatabase;
$manejador=new SeedDMS_Core_DatabaseAccess($driver,$host,$user,$pass,$base);
  $estado=$manejador->connect();
  //echo "Conectado: ".$estado;
  if($estado!=1)
  {
    echo "comprobarExistencias.php: Error: no se pudo conectar a la BD para modificar a ";
	exit;
  } 
 
    $crearTablaInstancias="INSERT INTO `docentes` (`id`, `nombre`, `correo`, `telefono`) VALUES (NULL, '$nombre', '$correo', '$tel');";
	$resultado0=$manejador->getResult($crearTablaInstancias);
	   if(!$resultado0)
		{
			UI::exitError("anadirDocenteNuevo: No se pudo añadir","No se pudo crear la tabla  (instancias_procesos_formativos)");
		}

?>