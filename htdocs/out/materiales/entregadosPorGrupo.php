...
foreach ($resultado1 as $fila) 
{
	$arrayTmp=array();
	$idReceptor=$fila['id'];	
	$cantidadEntregada="SELECT SUM(cantidad_variada) FROM app_transaccion WHERE tipo_transaccion=1 AND id_item=$idItem AND id_grupo_receptor=$idReceptor AND (fecha BETWEEN '$fechaInicio' AND '$fechaFin');";
